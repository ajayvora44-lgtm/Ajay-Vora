...
const TaxAssistant: React.FC<TaxAssistantProps> = ({ onClose }) => {
  ...
  return (
    <div className="flex flex-col w-[calc(100vw-2rem)] sm:w-[440px] h-[580px] max-h-[85vh] bg-white rounded-[2rem] sm:rounded-[2.5rem] shadow-[0_40px_100px_-20px_rgba(0,0,0,0.35)] overflow-hidden border border-slate-200 animate-in slide-in-from-bottom-5 duration-500 relative ring-1 ring-slate-200/50">
      <div className="bg-slate-900 p-4 sm:p-5 text-white flex justify-between items-center shrink-0 shadow-2xl relative z-40">
        <div className="flex items-center space-x-3 sm:space-x-4">
          <div className="w-10 h-10 sm:w-11 sm:h-11 bg-indigo-600 rounded-xl sm:rounded-2xl flex items-center justify-center border border-white/10 shadow-lg ring-4 ring-indigo-500/10">
            <svg className="w-5 h-5 sm:w-6 sm:h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
            </svg>
          </div>
          <div>
            <span className="block font-serif font-bold text-base sm:text-lg tracking-tight leading-none mb-1">Advisor Command</span>
            <div className="flex items-center space-x-2">
              <span className="w-1.5 h-1.5 bg-indigo-500 rounded-full animate-pulse shadow-[0_0_8px_rgba(99,102,241,0.8)]"></span>
              <span className="text-[8px] sm:text-[9px] text-slate-400 font-black uppercase tracking-[0.2em] sm:tracking-[0.25em]">Secure Node: Active</span>
            </div>
          </div>
        </div>
        <button 
          ...
          className="hover:bg-white/20 p-2 sm:p-2.5 rounded-xl transition-all active:scale-90 flex items-center justify-center bg-white/10 border border-white/5 relative z-50 cursor-pointer shadow-inner"
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-8 sm:space-y-10 bg-slate-50/50 scrollbar-hide">
        {messages.map((msg, i) => (
          <div key={i} className={`flex flex-col animate-message ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
            <div className={`max-w-[95%] p-4 sm:p-6 rounded-2xl sm:rounded-3xl text-[13px] sm:text-[14px] shadow-sm relative ${
              msg.role === 'user' 
              ...
              {msg.role === 'assistant' && (
                 <div className="absolute -top-3 -left-1 text-[7px] sm:text-[8px] font-black bg-indigo-600 text-white px-2 py-0.5 rounded shadow-sm uppercase tracking-widest z-10">AI_Output</div>
              )}
              ...
              {msg.role === 'assistant' && msg.showConsultationButton && (
                <div className="mt-6 sm:mt-8 p-4 sm:p-6 bg-gradient-to-br from-indigo-600 to-indigo-900 rounded-[1.5rem] sm:rounded-[2rem] text-white shadow-2xl shadow-indigo-600/40 border border-indigo-400/30 overflow-hidden relative group/cta">
                  <p className="text-[9px] font-black mb-3 opacity-90 uppercase tracking-[0.3em]">Statutory Review Required</p>
                  <p className="text-[11px] sm:text-[12px] font-medium mb-5 opacity-90 leading-relaxed">This brief contains elements that mandate direct legal mapping by our Lead Advisory Partners.</p>
                  <button 
                    onClick={scrollToContact}
                    className="w-full py-3.5 sm:py-4.5 bg-white text-indigo-700 text-[10px] sm:text-[11px] font-black rounded-xl hover:bg-slate-50 transition-all flex items-center justify-center space-x-3 uppercase tracking-[0.2em] shadow-xl active:scale-95 group/btn border border-indigo-100"
                  >
              ...
              {msg.role === 'assistant' && msg.suggestions && msg.suggestions.length > 0 && (
                <div className="mt-6 sm:mt-8 pt-6 border-t border-slate-100">
                  <p className="text-[8px] sm:text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 opacity-70">Suggested Tracks</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    {msg.suggestions.map((s, idx) => (
                      <button
                        key={idx}
                        onClick={() => handleSend(s.query)}
                        className="px-3 py-3 rounded-xl bg-slate-50 border border-slate-200 hover:border-indigo-600 hover:bg-white hover:text-indigo-600 transition-all text-[9px] sm:text-[10px] font-bold text-slate-600 flex items-center space-x-3 shadow-sm group/path text-left"
                      >
                        <span className="text-base group-hover/path:scale-125 transition-transform">{s.icon}</span>
        ...
        {isLoading && (
          <div className="flex items-start space-x-3 sm:space-x-4">
             <div className="w-9 h-9 sm:w-10 sm:h-10 bg-slate-900 rounded-xl flex items-center justify-center shadow-xl border border-white/5 shrink-0">
                <div className="w-1.5 h-1.5 bg-indigo-500 rounded-full animate-ping"></div>
             </div>
             <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xl shadow-slate-200/20 text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] animate-pulse">
      ...
      <div className="p-4 sm:p-5 bg-white border-t border-slate-100 shrink-0 shadow-[0_-10px_30px_rgba(0,0,0,0.02)]">
        <div className="relative group">
          <input 
            ...
            className="w-full pl-5 pr-14 py-4 md:py-5 rounded-xl md:rounded-2xl border border-slate-200 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 transition-all text-sm font-medium placeholder:text-slate-300 shadow-inner bg-slate-50/50" 
          />
          <button 
            onClick={() => handleSend()} 
            disabled={isLoading || !input.trim()} 
            className="absolute right-1.5 top-1/2 -translate-y-1/2 p-2.5 bg-indigo-600 text-white rounded-lg md:rounded-xl hover:bg-indigo-700 disabled:bg-slate-200 transition-all active:scale-95 shadow-xl shadow-indigo-600/30"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" /></svg>
          </button>
        </div>
        <p className="text-[7px] text-center text-slate-400 mt-4 font-black uppercase tracking-[0.4em] opacity-80 leading-none">Intelligence Hub • Sovereign Protocol</p>
      </div>
    </div>
  );
};
